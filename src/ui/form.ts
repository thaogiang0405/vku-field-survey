...
const CATEGORIES: { value: CategoryType; label: string }[] = [
  { value: 'Hardware', label: 'Thiết bị' }, { value: 'Projector', label: 'Máy chiếu' }, { value: 'AC', label: 'Điều hòa' }, { value: 'Electrical', label: 'Hệ thống điện' }, { value: 'Furniture', label: 'Nội thất' },
];
...
export function createInspectionForm(onInspectionSaved: () => void | Promise<void>): HTMLDivElement {
  ...
  container.innerHTML = `<div class="form-card"><div class="card-heading"><div class="heading-icon" aria-hidden="true">▣</div><div><p class="eyebrow">Khảo sát hiện trường</p><h2>Tạo phiếu khảo sát mới</h2></div></div><form id="inspectionForm" class="inspection-form" novalidate>
    <fieldset class="form-section"><legend>⌖ Thông tin địa điểm</legend><div class="form-group"><label for="building">Tòa nhà *</label><select id="building" name="building" required><option value="">Chọn tòa nhà</option>${BUILDINGS.map((building) => `<option value="${building}">${building}</option>`).join('')}</select><small class="error-message"></small></div><div class="form-row"><div class="form-group"><label for="floor">Tầng *</label><input id="floor" name="floor" type="number" min="1" max="10" required placeholder="Ví dụ: 1"/><small class="error-message"></small></div><div class="form-group"><label for="room">Phòng *</label><input id="room" name="room" required placeholder="Ví dụ: A101" list="room-list"/><datalist id="room-list">${ROOMS.map((room) => `<option value="${room}"></option>`).join('')}</datalist><small class="error-message"></small></div></div></fieldset>
    <fieldset class="form-section"><legend>⌘ Hạng mục kiểm tra</legend><div class="form-group"><label for="category">Hạng mục *</label><select id="category" name="category" required><option value="">Chọn hạng mục</option>${CATEGORIES.map((category) => `<option value="${category.value}">${category.label}</option>`).join('')}</select><small class="error-message"></small></div></fieldset>
